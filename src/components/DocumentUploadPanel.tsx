...
type DocumentUploadPanelProps = {
  compact?: boolean;
  onSuccess?: (redirectUrl: string, documentId?: string) => void;
};
...
export function DocumentUploadPanel({ compact = false, onSuccess }: DocumentUploadPanelProps) {
  ...
  const uploadFile = async (file?: File | null) => {
    ...
    try {
      ...
      window.setTimeout(() => {
        if (onSuccess) {
          onSuccess(payload.redirectUrl!, payload.documentId);
        } else {
  ...
  return (
    <section className={compact ? "" : "rounded-2xl border border-slate-200 bg-white p-5 shadow-soft"}>
      <div
        onDragOver={(event) => {
        ...
        className={`flex ${compact ? "min-h-[300px]" : "min-h-[390px]"} flex-col items-center justify-center rounded-2xl border-2 border-dashed px-6 py-10 text-center transition ${
      ...
      >
        <input ref={inputRef} type="file" accept="application/pdf,.pdf" className="hidden" onChange={(event) => void uploadFile(event.target.files?.item(0))} />
        ...
        <h1 className={`${compact ? "text-xl" : "text-2xl"} font-bold text-slate-950`}>{stateCopy[state].title}</h1>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          {!done && !hasError && (
            <button onClick={() => inputRef.current?.click()} disabled={busy} className="rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-70">
              选择文件
            </button>
          )}
          {hasError && (
